# Requires: discord.py >= 2.6.0 (Components v2), python-dotenv, requests
import discord
...
# NOTE: DISCORD_TOKEN must NOT have a hardcoded fallback. Secrets should
# never live in source code. If it's missing, we fail fast instead of
# silently booting with a baked-in token.
# ─────────────────────────────────────────────────────────────────────────
...
HOST_MOTD = os.getenv('HOST_MOTD', 'bash <(curl -fsSL https://raw.githubusercontent.com/MrRangerXD/test1/refs/heads/main/AetherCloud)')
BOT_VERSION = os.getenv('BOT_VERSION', '1.0 PRO')
BOT_DEVELOPER = os.getenv('BOT_DEVELOPER', 'ZenseiBabe')
BOT_THUMBNAIL_URL = os.getenv('BOT_THUMBNAIL_URL', 'https://i.imgur.com/n2dkdyR.png')
BOT_ICON_URL = os.getenv('BOT_ICON_URL', 'https://i.imgur.com/n2dkdyR.png')
...
def init_db():
    ...
    # Security migration: legacy versions stored VPS root passwords.
    # Scrub any legacy values and remove the column when SQLite supports it.
    if 'root_password' in columns:
        try:
            cur.execute('UPDATE vps SET root_password = NULL')
            cur.execute('ALTER TABLE vps DROP COLUMN root_password')
            logger.warning('Legacy VPS root_password column removed from database.')
        except sqlite3.OperationalError as exc:
            logger.warning('Could not drop legacy root_password column: %s', exc)
...
def save_vps_data():
    """Persist VPS metadata without ever persisting root passwords."""
    conn = get_db()
    cur = conn.cursor()
    for user_id, vps_list in vps_data.items():
        for vps in vps_list:
            ...
            # Defense in depth: discard any legacy/in-memory password field.
            vps.pop('root_password', None)
            if 'id' not in vps or vps['id'] is None:
                cur.execute("""INSERT INTO vps
                    (user_id, node_id, container_name, ram, cpu, storage, config, os_version,
                     status, suspended, whitelisted, created_at, shared_with, suspension_history, expiration_date)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                    (user_id, node_id, vps['container_name'], vps['ram'], vps['cpu'], vps['storage'],
                     vps['config'], os_ver, vps['status'], suspended_int, whitelisted_int, created_at,
                     shared_json, history_json, expiration_date))
                ...
                cur.execute("""UPDATE vps SET
                    ...
                    shared_with = ?, suspension_history = ?, expiration_date = ?
                    WHERE id = ?""",
                    (user_id, node_id, vps['container_name'], vps['ram'], vps['cpu'], vps['storage'],
                     vps['config'], os_ver, vps['status'], suspended_int, whitelisted_int, shared_json,
                     history_json, expiration_date, vps['id']))
    conn.commit()
    conn.close()
...
def allocate_ports(user_id: str, amount: int):
    ...
    cur = conn.cursor()
    cur.execute(
        'INSERT OR REPLACE INTO port_allocations (user_id, allocated_ports) '
...
def deallocate_ports(user_id: str, amount: int):
    """
    FIX: SQLite has no GREATEST() function (that's MySQL/Postgres syntax) —
    the original query would raise sqlite3.OperationalError on every call.
    Clamp to zero in Python instead.
    """
    conn = get_db()
    cur = conn.cursor()
    cur.execute('SELECT allocated_ports FROM port_allocations WHERE user_id = ?', (user_id,))
    ...
    cur.execute(
        'INSERT OR REPLACE INTO port_allocations (user_id, allocated_ports) VALUES (?, ?)',
        (user_id, new_value)
    )
    conn.commit()
    conn.close()
...
# FIX: vps_data is read/mutated from both the Discord event loop (command
# handlers) and a background OS thread (resource_monitor / expiration
# checks). A plain dict/list has no locking, so concurrent mutation during
# iteration can raise "dictionary changed size during iteration" or produce
# torn reads. This lock serializes access from both sides.
vps_data_lock = threading.RLock()
...
def reserve_vps_id() -> int:
    """
    FIX: the original code computed the next VPS id as MAX(id)+1 in Python,
    which is a classic TOCTOU race — two people running !create at nearly
    the same time can both compute the same id before either INSERT
    commits, leading to a duplicate/incorrect id or a UNIQUE constraint
    failure on container_name.

    Instead, we atomically INSERT a placeholder row right away to claim a
    real AUTOINCREMENT id from SQLite, and return that id. The caller later
    fills in the real details and calls save_vps_data(), which will UPDATE
    this row (since it already has an 'id') instead of inserting a new one.
    """
    conn = get_db()
    cur = conn.cursor()
    placeholder_name = f"__reserved__{secrets.token_hex(8)}"
    cur.execute("""INSERT INTO vps
        (user_id, node_id, container_name, ram, cpu, storage, config, os_version,
         status, suspended, whitelisted, created_at, shared_with, suspension_history, expiration_date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        ('0', 1, placeholder_name, '0GB', '0', '0GB', 'reserved', 'ubuntu:22.04',
         'stopped', 0, 0, datetime.now().isoformat(), '[]', '[]', None))
...
# Components v2 requires discord.py 2.6+
if not hasattr(discord.ui, 'LayoutView') or not hasattr(discord.ui, 'Container'):
    raise RuntimeError('Components v2 requires discord.py >= 2.6.0. Upgrade with: pip install -U \"discord.py>=2.6.0\"')
...
def sanitize_username_for_container(username: str) -> str:
    ...
    # FIX: if sanitization strips everything (e.g. an all-emoji/unicode
    # username), fall back to a safe generated name instead of producing
    # an empty or invalid container name.
    if not sanitized:
        sanitized = f"user-{secrets.token_hex(3)}"
    return sanitized
...
Subsystem sftp /usr/lib/openssh/sftp-server"""
        ...
        # Password is intentionally returned only to the caller and is never stored.
...
def _embed_color_value(embed):
    try:
        return embed.colour.value or COLOR_PRIMARY
    except Exception:
        return COLOR_PRIMARY


def _render_embed_into_v2_container(container, embed):
    """Render legacy Embed data as one compact Components v2 TextDisplay."""
    ...
    content = "\n\n".join(parts)
    # LayoutView has a 4000-character total display-content limit. Keep a little
    # headroom so edits remain valid even with Discord-side formatting changes.
    if len(content) > 3900:
        content = content[:3897] + "..."
    if content:
        container.add_item(discord.ui.TextDisplay(content))
    try:
        container.accent_color = _embed_color_value(embed)
    except Exception:
        pass
...
async def send_ui(destination, content=None, *, embed=None, view=None, **kwargs):
    # Plain prompts are intentionally left as normal Discord content; any
    # embed-based UI is rendered through Components v2 below.
    if embed is not None or view is not None:
        view = ensure_v2_view(embed, view)
        return await destination.send(view=view, **kwargs)
    return await destination.send(content, **kwargs)


async def edit_ui(target, *, embed=None, view=None, **kwargs):
    if embed is not None or view is not None:
        view = ensure_v2_view(embed, view)
    if hasattr(target, 'edit_original_response'):
        return await target.edit_original_response(view=view, **kwargs)
    if hasattr(target, 'edit_message'):
        return await target.edit_message(view=view, **kwargs)
    if hasattr(target, 'edit'):
        return await target.edit(view=view, **kwargs)
    raise TypeError(f"Unsupported edit target: {type(target)!r}")
...
async def get_host_stats(node_id: int) -> Dict:
    """
    FIX: this function was defined TWICE in the original file (once here,
    once again further down near the vps-list command). The second
    definition silently shadowed this one. Having only one definition
    avoids the risk of the two copies drifting apart after future edits.
    """
    node = get_node(node_id)
    if node['is_local']:
        return {
...
#
# FIX: the original code called `asyncio.run(bot.fetch_user(...))` and
# `asyncio.run(send_ui(...))` directly from a plain threading.Thread. These
# discord.py calls are bound to the bot's own event loop/HTTP session; a
# brand-new loop created by asyncio.run() in a different OS thread is not
# safe to use for them and can raise RuntimeError or behave unpredictably.
# The fix: any coroutine that touches the bot/Discord API is scheduled onto
# the bot's real running loop via asyncio.run_coroutine_threadsafe(), from
# this background thread.
# ─────────────────────────────────────────────────────────────────────────

async def _notify_expired_vps(user_id: str, container_name: str, expiration_dt: datetime):
    try:
        ...
        await send_ui(owner, embed=dm_embed)
    except Exception as e:
        logger.debug(f"Failed to notify user {user_id} about expiration: {e}")

async def _notify_expiring_soon(user_id: str, container_name: str, expiration_dt: datetime, days_remaining: int):
    try:
        owner = await bot.fetch_user(int(user_id))
        ...
        logger.debug(f"Failed to notify user {user_id} about upcoming expiration: {e}")

async def _stop_container_for_expiration(container_name: str, node_id: int):
    """Runs the actual lxc stop call on the bot's loop for consistency."""
    await execute_lxc(container_name, f"stop {container_name}", node_id=node_id)

def _run_on_bot_loop(coro, timeout: float = 30.0):
    """Schedule a coroutine on the bot's real event loop from a worker thread and wait for it."""
    loop = getattr(bot, 'loop', None)
    if loop is None or loop.is_closed():
        logger.debug("Bot loop not available yet; skipping scheduled coroutine.")
        return None
    future = asyncio.run_coroutine_threadsafe(coro, loop)
    try:
        return future.result(timeout=timeout)
    except Exception as e:
        logger.error(f"Error running scheduled coroutine on bot loop: {e}")
        return None
...
def check_vps_expiration():
    ...
    try:
        ...
        # FIX: snapshot with list() so mutation from the main thread while
        # we iterate here doesn't raise "dictionary changed size during
        # iteration".
        with vps_data_lock:
            snapshot = {uid: list(lst) for uid, lst in vps_data.items()}

        for user_id, vps_list in snapshot.items():
            for vps in vps_list:
                if vps.get('expiration_date'):
                    ...
                    if days_remaining < 0:
                        if not vps.get('suspended', False):
                            try:
                                _run_on_bot_loop(_stop_container_for_expiration(container_name, node_id))
                                with vps_data_lock:
                                    vps['status'] = 'stopped'
                                    vps['suspended'] = True
                                ...
                                _run_on_bot_loop(_notify_expired_vps(user_id, container_name, expiration_dt), timeout=15)
                            except Exception as e:
                    ...
                    # Send warning if expiring soon
                        ...
                        if user_id not in warned_users:
                            _run_on_bot_loop(_notify_expiring_soon(user_id, container_name, expiration_dt, days_remaining), timeout=15)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        await send_ui(ctx, embed=create_error_embed("Missing Argument", f"Please check command usage with `{PREFIX}help`."))
    elif isinstance(error, commands.BadArgument):
        await send_ui(ctx, embed=create_error_embed("Invalid Argument", "Please check your input and try again."))
    elif isinstance(error, commands.CheckFailure):
        error_msg = str(error) if str(error) else "You need admin permissions for this command. Contact support."
        await send_ui(ctx, embed=create_error_embed("Access Denied", error_msg))
    elif isinstance(error, discord.NotFound):
        await send_ui(ctx, embed=create_error_embed("Error", "The requested resource was not found. Please try again."))
    else:
        logger.error(f"Command error: {error}")
        await send_ui(ctx, embed=create_error_embed("System Error", "An unexpected error occurred. Support has been notified."))
...
async def set_threshold(ctx, cpu: int, ram: int):
    ...
    if cpu < 0 or ram < 0:
        await send_ui(ctx, embed=create_error_embed("Invalid Thresholds", "Thresholds must be non-negative."))
        return
    CPU_THRESHOLD = cpu
    RAM_THRESHOLD = ram
...
async def set_status(ctx, activity_type: str, *, name: str):
    ...
    if activity_type.lower() not in types:
        await send_ui(ctx, embed=create_error_embed("Invalid Type", "Valid types: playing, watching, listening, streaming"))
        return
...
class NodeSelectView(AppLayoutView):
    ...
    async def select_node(self, interaction: discord.Interaction):
        if str(interaction.user.id) != str(self.ctx.author.id):
            await send_ui(interaction.response, embed=create_error_embed("Access Denied", "Only the command author can select."), ephemeral=True)
            return
        node_id = int(self.select.values[0])
        self.select.disabled = True
        await edit_ui(interaction.response, view=self)
...
class OSSelectView(AppLayoutView):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        await edit_ui(interaction.response, embed=creating_embed, view=self)
        user_id = str(self.user.id)
        # Create shorter container name with GLOBAL VPS ID
        username = self.user.name.lower().replace(" ", "-")[:15]  # Limit to 15 chars

        # FIX: reserve the global VPS id atomically via SQLite AUTOINCREMENT
        # instead of computing MAX(id)+1 in Python, which was racy under
        # concurrent VPS creation (see reserve_vps_id() docstring above).
        global_vps_id = reserve_vps_id()
        ...
        try:
            ...
            await send_ui(interaction.followup, embed=success_embed)
...
• Check logs with: `journalctl -xe`"""
            ...
            try:
                await send_ui(self.user, embed=dm_embed)
            except discord.Forbidden:
                await send_ui(self.ctx, embed=create_info_embed("Notification Failed", f"Couldn't send DM to {self.user.mention}. Please ensure DMs are enabled."))
        except Exception as e:
            error_embed = create_error_embed("Creation Failed", f"Error: {str(e)}")
            await send_ui(interaction.followup, embed=error_embed)

@bot.command(name='create')
@is_admin()
async def create_vps(ctx, ram: int, cpu: int, disk: int, user: discord.Member, expiry_days: int = None):
    if ram <= 0 or cpu <= 0 or disk <= 0:
        await send_ui(ctx, embed=create_error_embed("Invalid Specs", "RAM, CPU, and Disk must be positive integers."))
        return

    # Validate expiry_days if provided
    if expiry_days is not None and expiry_days <= 0:
        await send_ui(ctx, embed=create_error_embed("Invalid Expiry Days", "Expiry days must be a positive integer."))
        return
...
class ReinstallOSSelectView(AppLayoutView):
    ...
    async def select_os(self, interaction: discord.Interaction):
        ...
        try:
            ...
            add_field(success_embed, "Features", "Nesting, Privileged, FUSE, Kernel Modules (Docker Ready), Unprivileged Ports from 0", False)
            add_field(success_embed, "Disk Note", "Run `sudo resize2fs /` inside VPS if needed to expand filesystem.", False)
            await send_ui(interaction.followup, embed=success_embed, ephemeral=True)
...
class ManageView(AppLayoutView):
    ...
    async def select_vps(self, interaction: discord.Interaction):
        if str(interaction.user.id) != self.user_id and not self.is_admin:
            await send_ui(interaction.response, embed=create_error_embed("Access Denied", "This is not your VPS!"), ephemeral=True)
            return
        ...
        self.clear_items()
        self.add_action_buttons()
        await edit_ui(interaction, embed=new_embed, view=self)
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if str(interaction.user.id) != self.user_id and not self.is_admin:
            await send_ui(interaction.followup, embed=create_error_embed("Access Denied", "This is not your VPS!"), ephemeral=True)
            return
        if self.selected_index is None:
            await send_ui(interaction.followup, embed=create_error_embed("No VPS Selected", "Please select a VPS first."), ephemeral=True)
            return
        ...
        if suspended and not self.is_admin and action != 'stats':
            await send_ui(interaction.followup, embed=create_error_embed("Access Denied", "This VPS is suspended. Contact an admin to unsuspend."), ephemeral=True)
            return
        ...
        if action == 'stats':
            try:
                ...
                await send_ui(interaction.followup, embed=stats_embed, ephemeral=True)
            except Exception as e:
                await send_ui(interaction.followup, embed=create_error_embed("Stats Failed", str(e)), ephemeral=True)
            return
        if action == 'reinstall':
            if self.is_shared or self.is_admin:
                await send_ui(interaction.followup, embed=create_error_embed("Access Denied", "Only the VPS owner can reinstall!"), ephemeral=True)
                return
            if suspended:
                await send_ui(interaction.followup, embed=create_error_embed("Cannot Reinstall", "Unsuspend the VPS first."), ephemeral=True)
                return
            ...
            class ConfirmView(AppLayoutView):
                ...
                async def confirm(self, inter: discord.Interaction, item: discord.ui.Button):
                    ...
                    try:
                        await send_ui(inter.followup, embed=create_info_embed("Deleting Container", f"Forcefully removing container `{self.container_name}`..."), ephemeral=True)
                        ...
                        await send_ui(inter.followup, embed=create_info_embed("Select OS", "Choose the new OS for reinstallation."), view=os_view, ephemeral=True)
                    except Exception as e:
                        await send_ui(inter.followup, embed=create_error_embed("Delete Failed", f"Error: {str(e)}"), ephemeral=True)
                ...
                async def cancel(self, inter: discord.Interaction, item: discord.ui.Button):
                    ...
                    await edit_ui(inter.response, embed=new_embed, view=self.parent_view)

            await send_ui(interaction.followup, embed=confirm_embed, view=ConfirmView(self, container_name, self.owner_id, actual_idx, ram_gb, cpu, storage_gb, node_id), ephemeral=True)
            return
        ...
        if action == 'start':
            try:
                ...
                if current_status == 'running':
                    await send_ui(interaction.followup, embed=create_info_embed("Already Running", f"VPS `{container_name}` is already running."), ephemeral=True)
                    return
                ...
                await send_ui(interaction.followup, embed=create_success_embed("VPS Started", f"VPS `{container_name}` is now running! Re-added {readded} port forwards."), ephemeral=True)
            except Exception as e:
                ...
                if "already running" in error_str:
                    target_vps["status"] = "running"
                    save_vps_data_immediate()
                    await send_ui(interaction.followup, embed=create_success_embed("VPS Started", f"VPS `{container_name}` is running!"), ephemeral=True)
                else:
                    await send_ui(interaction.followup, embed=create_error_embed("Start Failed", str(e)), ephemeral=True)
        elif action == 'stop':
            try:
                # Check current status to avoid "not running" error
                current_status = target_vps.get('status', 'stopped')
                if current_status == 'stopped':
                    await send_ui(interaction.followup, embed=create_info_embed("Already Stopped", f"VPS `{container_name}` is already stopped."), ephemeral=True)
                    return

                await execute_lxc(container_name, f"stop {container_name}", timeout=120, node_id=node_id)
                target_vps["status"] = "stopped"
                save_vps_data_immediate()
                await send_ui(interaction.followup, embed=create_success_embed("VPS Stopped", f"VPS `{container_name}` has been stopped!"), ephemeral=True)
            except Exception as e:
                ...
                if "not running" in error_str or "is not running" in error_str:
                    target_vps["status"] = "stopped"
                    save_vps_data_immediate()
                    await send_ui(interaction.followup, embed=create_success_embed("VPS Stopped", f"VPS `{container_name}` is stopped!"), ephemeral=True)
                else:
                    await send_ui(interaction.followup, embed=create_error_embed("Stop Failed", str(e)), ephemeral=True)
        elif action == 'tmate':
            if suspended:
                await send_ui(interaction.followup, embed=create_error_embed("Access Denied", "Cannot access suspended VPS."), ephemeral=True)
                return
            await send_ui(interaction.followup, embed=create_info_embed("SSH Access", "Generating SSH connection..."), ephemeral=True)
            try:
                # Check if VPS is running first
                current_status = target_vps.get('status', 'stopped')
                if current_status != 'running':
                    await send_ui(interaction.followup, embed=create_error_embed("VPS Not Running", "Start the VPS before accessing SSH."), ephemeral=True)
                    return
                ...
                try:
                    ...
                    if "not running" in error_str or "is not running" in error_str:
                        await send_ui(interaction.followup, embed=create_info_embed("Starting VPS", "VPS is stopped. Starting it..."), ephemeral=True)
                        await execute_lxc(container_name, f"start {container_name}", node_id=node_id)
                        target_vps["status"] = "running"
                        save_vps_data_immediate()
                ...
                if not tmate_installed:
                    await send_ui(interaction.followup, embed=create_info_embed("Installing SSH", "Installing tmate..."), ephemeral=True)
                    try:
                        ...
                        await send_ui(interaction.followup, embed=create_success_embed("Installed", "SSH service installed!"), ephemeral=True)
                    except Exception as install_err:
                        await send_ui(interaction.followup, embed=create_error_embed("Install Failed", str(install_err)), ephemeral=True)
                        return
                ...
                if ssh_url:
                    try:
                        ...
                        await send_ui(interaction.user, embed=ssh_embed)
                        await send_ui(interaction.followup, embed=create_success_embed("SSH Sent", f"Check your DMs for SSH link! Session: {session_name}"), ephemeral=True)
                    except discord.Forbidden:
                        await send_ui(interaction.followup, embed=create_error_embed("DM Failed", "Enable DMs to receive SSH link!"), ephemeral=True)
                else:
                    await send_ui(interaction.followup, embed=create_error_embed("SSH Failed", "No SSH URL generated."), ephemeral=True)
            except Exception as e:
                await send_ui(interaction.followup, embed=create_error_embed("SSH Error", str(e)), ephemeral=True)
        elif action == 'regen_password':
            if suspended:
                await send_ui(interaction.followup, embed=create_error_embed("Access Denied", "Cannot regenerate password for suspended VPS."), ephemeral=True)
                return
            try:
                ...
                if success:
                    ...
                    await send_ui(interaction.followup, embed=password_embed, ephemeral=True)
                else:
                    await send_ui(interaction.followup, embed=create_error_embed("Regen Failed", str(result)), ephemeral=True)
            except Exception as e:
                await send_ui(interaction.followup, embed=create_error_embed("Error", f"Failed to regenerate password: {str(e)}"), ephemeral=True)
        new_embed = await self.create_vps_embed(self.selected_index)
        await edit_ui(interaction, embed=new_embed, view=self)
...
async def manage_vps(ctx, user: discord.Member = None):
    if user:
        if str(ctx.author.id) != str(MAIN_ADMIN_ID) and str(ctx.author.id) not in admin_data.get("admins", []):
            await send_ui(ctx, embed=create_error_embed("Access Denied", "Only admins can manage other users' VPS."))
            return
        user_id = str(user.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
            await send_ui(ctx, embed=create_error_embed("No VPS Found", f"{user.mention} doesn't have any {BOT_NAME} VPS."))
            return
        view = ManageView(str(ctx.author.id), vps_list, is_admin=True, owner_id=user_id)
        await send_ui(ctx, embed=create_info_embed(f"Managing {user.name}'s VPS", f"Managing VPS for {user.mention}"), view=view)
    else:
        user_id = str(ctx.author.id)
        vps_list = vps_data.get(user_id, [])
        if not vps_list:
...
async def vps_list(ctx, node_id: int = 1):
    node = get_node(node_id)
    if not node:
        await send_ui(ctx, embed=create_error_embed("Node Not Found", f"Node ID {node_id} not found."))
        return
...
async def manage_shared_vps(ctx, owner: discord.Member, vps_number: int):
    ...
    if owner_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[owner_id]):
        await send_ui(ctx, embed=create_error_embed("Invalid VPS", "Invalid VPS number or owner doesn't have a VPS."))
        return
    vps = vps_data[owner_id][vps_number - 1]
    if user_id not in vps.get("shared_with", []):
        await send_ui(ctx, embed=create_error_embed("Access Denied", "You do not have access to this VPS."))
        return
...
async def share_user(ctx, shared_user: discord.Member, vps_number: int):
    ...
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await send_ui(ctx, embed=create_error_embed("Invalid VPS", "Invalid VPS number or you don't have a VPS."))
        return
    vps = vps_data[user_id][vps_number - 1]
    if "shared_with" not in vps:
        vps["shared_with"] = []
    if shared_user_id in vps["shared_with"]:
        await send_ui(ctx, embed=create_error_embed("Already Shared", f"{shared_user.mention} already has access to this VPS!"))
        return
    vps["shared_with"].append(shared_user_id)
    save_vps_data()
    await send_ui(ctx, embed=create_success_embed("VPS Shared", f"VPS #{vps_number} shared with {shared_user.mention}!"))
    try:
        await send_ui(shared_user, embed=create_embed("VPS Access Granted", f"You have access to VPS #{vps_number} from {ctx.author.mention}. Use `{PREFIX}manage-shared {ctx.author.mention} {vps_number}`", 0x00ff88))
    except discord.Forbidden:
        await send_ui(ctx, embed=create_info_embed("Notification Failed", f"Could not DM {shared_user.mention}"))
...
async def revoke_share(ctx, shared_user: discord.Member, vps_number: int):
    ...
    if shared_user_id not in vps["shared_with"]:
        await send_ui(ctx, embed=create_error_embed("Not Shared", f"{shared_user.mention} doesn't have access to this VPS!"))
        return
    vps["shared_with"].remove(shared_user_id)
    save_vps_data()
    await send_ui(ctx, embed=create_success_embed("Access Revoked", f"Access to VPS #{vps_number} revoked from {shared_user.mention}!"))
    try:
        await send_ui(shared_user, embed=create_embed("VPS Access Revoked", f"Your access to VPS #{vps_number} by {ctx.author.mention} has been revoked.", 0xff3366))
    except discord.Forbidden:
        await send_ui(ctx, embed=create_info_embed("Notification Failed", f"Could not DM {shared_user.mention}"))
...
async def ports_add_user(ctx, amount: int, user: discord.Member):
    if amount <= 0:
        await send_ui(ctx, embed=create_error_embed("Invalid Amount", "Amount must be a positive integer."))
        return
    user_id = str(user.id)
    ...
    try:
        ...
        await send_ui(user, embed=dm_embed)
    except discord.Forbidden:
        await send_ui(ctx, embed=create_info_embed("DM Failed", f"Could not notify {user.mention} via DM."))
...
async def ports_revoke(ctx, forward_id: int):
    ...
    if success and user_id:
        try:
            ...
            pass
        await send_ui(ctx, embed=create_success_embed("Revoked", f"Port forward ID {forward_id} revoked."))
    else:
        await send_ui(ctx, embed=create_error_embed("Failed", "Port forward ID not found or removal failed."))
...
async def ports_command(ctx, subcmd: str = None, *args):
    ...
    if subcmd == 'add':
        if len(args) < 2:
            await send_ui(ctx, embed=create_error_embed("Usage", f"Usage: {PREFIX}ports add <vps_number> <vps_port>"))
            return
        try:
            vps_num = int(args[0])
            vps_port = int(args[1])
            ...
            await send_ui(ctx, embed=create_error_embed("Invalid Input", "VPS number and port must be positive integers (port: 1-65535)."))
            return
        vps_list = vps_data.get(user_id, [])
        if vps_num < 1 or vps_num > len(vps_list):
            await send_ui(ctx, embed=create_error_embed("Invalid VPS", f"Invalid VPS number (1-{len(vps_list)}). Use {PREFIX}myvps to list."))
            return
        vps = vps_list[vps_num - 1]
        container = vps['container_name']
        node_id = vps['node_id']
        if used >= allocated:
            await send_ui(ctx, embed=create_error_embed("Quota Exceeded", f"No available slots. Allocated: {allocated}, Used: {used}. Contact admin for more."))
            return
        ...
        if host_port:
            ...
            await send_ui(ctx, embed=create_error_embed("Failed", "Could not assign host port. Try again later."))
    elif subcmd == 'list':
        ...
        if len(args) < 1:
            await send_ui(ctx, embed=create_error_embed("Usage", f"Usage: {PREFIX}ports remove <forward_id>"))
            return
        try:
            fid = int(args[0])
        except ValueError:
            await send_ui(ctx, embed=create_error_embed("Invalid ID", "Forward ID must be an integer."))
            return
        ...
        if success:
            ...
            await send_ui(ctx, embed=create_error_embed("Not Found", "Forward ID not found. Use !ports list."))
    else:
...
class ConfirmDeleteView(AppLayoutView):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if str(interaction.user.id) != self.admin_id:
            await send_ui(interaction.response,
...
async def delete_vps(ctx, user: discord.Member, vps_number: int, *, reason: str = "No reason"):
    ...
    if user_id not in vps_data or vps_number < 1 or vps_number > len(vps_data[user_id]):
        await send_ui(ctx, embed=create_error_embed(
            "Invalid VPS",
    ...
    confirmation_msg = await send_ui(ctx, embed=confirm_embed, view=confirmation_view)
    ...
    await send_ui(ctx, embed=create_info_embed(
        "🗑️ Deleting VPS",
    ...
    # 1️⃣ Delete the infrastructure first. Preserve DB state on unknown failures.
    try:
        ...
        if any(x in err for x in ["not found", "does not exist", "no such container"]):
            node_result = "Container already absent; database cleanup is safe."
        else:
            await send_ui(ctx, embed=create_error_embed("Deletion Aborted", f"The container could not be deleted, so the VPS database record was preserved.\n\n**Reason:** {e}"))
            return
...
async def add_resources(ctx, vps_id: str, ram: int = None, cpu: int = None, disk: int = None):
    if ram is None and cpu is None and disk is None:
        await send_ui(ctx, embed=create_error_embed("Missing Parameters", "Please specify at least one resource to add (ram, cpu, or disk)"))
        return
    found_vps = None
    user_id = None
    vps_index = None
    for uid, vps_list in vps_data.items():
        ...
        if found_vps:
            break
    if not found_vps:
        await send_ui(ctx, embed=create_error_embed("VPS Not Found", f"No VPS found with ID: `{vps_id}`"))
        return
    ...
    if was_running:
        await send_ui(ctx, embed=create_info_embed("Stopping VPS", f"Stopping VPS `{vps_id}` to apply resource changes..."))
        try:
            # FIX: original code passed the literal string "stop {vps_id}"
            # (missing the f-prefix) which sent a broken lxc command.
            await execute_lxc(vps_id, f"stop {vps_id}", node_id=node_id)
            found_vps['status'] = 'stopped'
            save_vps_data()
        except Exception as e:
            await send_ui(ctx, embed=create_error_embed("Stop Failed", f"Error stopping VPS: {str(e)}"))
            return
    changes = []
    try:
        ...
        await send_ui(ctx, embed=create_error_embed("Resource Addition Failed", f"Error: {str(e)}"))
...
async def vps_status_cmd(ctx, container_name: str = None):
    """
    NEW: a lightweight per-VPS status command.
    The help menu previously advertised '{PREFIX}status <vps-id>' for this
    purpose, but the actual `status` command takes no arguments and shows
    the whole-system dashboard instead — calling it with a container name
    threw a TooManyArguments error. This command fills that documented gap.
    """
    user_id = str(ctx.author.id)
    is_admin_user = user_id == str(MAIN_ADMIN_ID) or user_id in admin_data.get("admins", [])

    if not container_name:
        await send_ui(ctx, embed=create_error_embed("Usage", f"Usage: `{PREFIX}vps-status <container_name>`\nUse `{PREFIX}myvps` to see your container names."))
        return
    ...
    if not found_vps:
        await send_ui(ctx, embed=create_error_embed("VPS Not Found", f"No VPS found with container name: `{container_name}`"))
        return

    if owner_id != user_id and not is_admin_user and user_id not in found_vps.get('shared_with', []):
        await send_ui(ctx, embed=create_error_embed("Access Denied", "You don't have access to this VPS."))
        return

    node_id = found_vps.get('node_id', 1)
    stats = await get_container_stats(container_name, node_id)
    stored_status = found_vps.get('status', 'unknown')
    suspended = found_vps.get('suspended', False)

    status_emoji = "🟢" if stored_status == 'running' and not suspended else "🟡" if suspended else "🔴"
    embed = create_info_embed(f"{status_emoji} VPS Status - {container_name}",
        f"**Status:** `{stored_status.upper()}`{' (SUSPENDED)' if suspended else ''}")
    add_field(embed, "CPU", f"{stats.get('cpu', 0):.1f}%", True)
    ram = stats.get('ram', {})
    add_field(embed, "Memory", f"{ram.get('used', 0)}/{ram.get('total', 0)} MB ({ram.get('pct', 0):.1f}%)", True)
    add_field(embed, "Disk", stats.get('disk', 'Unknown'), True)
    add_field(embed, "Uptime", stats.get('uptime', 'Unknown'), False)
    await send_ui(ctx, embed=embed)
...
async def admin_add(ctx, user: discord.Member):
    ...
    if user_id == str(MAIN_ADMIN_ID):
        await send_ui(ctx, embed=create_error_embed("Already Admin", "This user is already the main admin!"))
        return
    if user_id in admin_data.get("admins", []):
        await send_ui(ctx, embed=create_error_embed("Already Admin", f"{user.mention} is already an admin!"))
        return
    admin_data["admins"].append(user_id)
    save_admin_data()
    await send_ui(ctx, embed=create_success_embed("Admin Added", f"{user.mention} is now an admin!"))
    try:
        await send_ui(user, embed=create_embed("🎉 Admin Role Granted", f"You are now an admin by {ctx.author.mention}", 0x00ff88))
    except discord.Forbidden:
        await send_ui(ctx, embed=create_info_embed("Notification Failed", f"Could not DM {user.mention}"))
...
async def admin_remove(ctx, user: discord.Member):
    user_id = str(user.id)
    if user_id == str(MAIN_ADMIN_ID):
        await send_ui(ctx, embed=create_error_embed("Cannot Remove", "You cannot remove the main admin!"))
        return
    if user_id not in admin_data.get("admins", []):
        await send_ui(ctx, embed=create_error_embed("Not Admin", f"{user.mention} is not an admin!"))
        return
    admin_data["admins"].remove(user_id)
    save_admin_data()
    await send_ui(ctx, embed=create_success_embed("Admin Removed", f"{user.mention} is no longer an admin!"))
    try:
        await send_ui(user, embed=create_embed("⚠️ Admin Role Revoked", f"Your admin role was removed by {ctx.author.mention}", 0xff3366))
    except discord.Forbidden:
        await send_ui(ctx, embed=create_info_embed("Notification Failed", f"Could not DM {user.mention}"))
...
async def restart_vps(ctx, container_name: str):
    node_id = find_node_id_for_container(container_name)
    await send_ui(ctx, embed=create_info_embed("Restarting VPS", f"Restarting VPS `{container_name}`..."))
    try:
        ...
        await apply_internal_permissions(container_name, node_id)
        await recreate_port_forwards(container_name)
        await send_ui(ctx, embed=create_success_embed("VPS Restarted", f"VPS `{container_name}` has been restarted successfully!"))
    except Exception as e:
        await send_ui(ctx, embed=create_error_embed("Restart Failed", f"Error: {str(e)}"))

@bot.command(name='exec')
@is_admin()
async def execute_command(ctx, container_name: str, *, command: str):
    node_id = find_node_id_for_container(container_name)
    await send_ui(ctx, embed=create_info_embed("Executing Command", f"Running command in VPS `{container_name}`..."))
    try:
        ...
        await send_ui(ctx, embed=create_error_embed("Execution Failed", f"Error: {str(e)}"))
    ...
    class ConfirmView(AppLayoutView):
        ...
        async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
            ...
            try:
                ...
                await send_ui(interaction.followup, embed=embed)
            except Exception as e:
        ...
        async def cancel(self, interaction: discord.Interaction, item: discord.ui.Button):
            await edit_ui(interaction.response, embed=create_info_embed("Operation Cancelled", "The stop all VPS operation has been cancelled."))

    await send_ui(ctx, embed=embed, view=ConfirmView())
...
async def resource_monitor_control(ctx, action: str = "status"):
    ...
    if action.lower() == "status":
        ...
        resource_monitor_active = True
        await send_ui(ctx, embed=create_success_embed("Resource Monitor Enabled", "Resource monitoring has been enabled."))
    elif action.lower() == "disable":
        resource_monitor_active = False
        await send_ui(ctx, embed=create_warning_embed("Resource Monitor Disabled", "Resource monitoring has been disabled."))
    else:
        await send_ui(ctx, embed=create_error_embed("Invalid Action", f"Use: `{PREFIX}cpu-monitor <status|enable|disable>`"))

@bot.command(name='resize-vps')
@is_admin()
async def resize_vps(ctx, container_name: str, ram: int = None, cpu: int = None, disk: int = None):
    if ram is None and cpu is None and disk is None:
        await send_ui(ctx, embed=create_error_embed("Missing Parameters", "Please specify at least one resource to resize (ram, cpu, or disk)"))
        return
    found_vps = None
    user_id = None
    vps_index = None
    ...
    disk_changed = disk is not None
    if was_running:
        await send_ui(ctx, embed=create_info_embed("Stopping VPS", f"Stopping VPS `{container_name}` to apply resource changes..."))
        try:
            await execute_lxc(container_name, f"stop {container_name}", node_id=node_id)
            found_vps['status'] = 'stopped'
            save_vps_data()
    ...
    try:
        ...
        await send_ui(ctx, embed=create_error_embed("Resize Failed", f"Error: {str(e)}"))
...
async def clone_vps(ctx, container_name: str, new_name: str = None):
    ...
    node_id = find_node_id_for_container(container_name)
    await send_ui(ctx, embed=create_info_embed("Cloning VPS", f"Cloning VPS `{container_name}` to `{new_name}`..."))
    try:
        found_vps = None
        user_id = None
        for uid, vps_list in vps_data.items():
        ...
        new_vps.pop('root_password', None)
        ...
        await send_ui(ctx, embed=create_error_embed("Clone Failed", f"Error: {str(e)}"))
...
async def migrate_vps(ctx, container_name: str, target_node_id: int):
    ...
    if node_id == target_node_id:
        await send_ui(ctx, embed=create_error_embed("Invalid Migration", "Source and target nodes are the same. Choose a different node."))
        return
    target_node = get_node(target_node_id)
    if not target_node:
        await send_ui(ctx, embed=create_error_embed("Invalid Node", "Target node not found."))
        return
    await send_ui(ctx, embed=create_info_embed("Migrating VPS", f"Migrating VPS `{container_name}` to node {target_node['name']}..."))
    temp_name = f"{BOT_NAME.lower()}-{container_name}-migration-{int(time.time())}"
    rollback_name = f"{BOT_NAME.lower()}-{container_name}-rollback-{int(time.time())}"
    staged = False
    rollback_created = False
    try:
        # Stop the source and make a local rollback copy before any destructive cut-over.
        await execute_lxc(container_name, f"stop {container_name}", node_id=node_id)
        await execute_lxc(container_name, f"copy {container_name} {rollback_name} -s {DEFAULT_STORAGE_POOL}", node_id=node_id)
        rollback_created = True

        # Stage the migration on the target.
        await execute_lxc(container_name, f"copy {container_name} {temp_name} -s {DEFAULT_STORAGE_POOL}", node_id=target_node_id)
        staged = True
        await execute_lxc(temp_name, f"info {temp_name}", node_id=target_node_id)
        await apply_lxc_config(temp_name, target_node_id)
        await apply_internal_permissions(temp_name, target_node_id)
        await execute_lxc(temp_name, f"start {temp_name}", node_id=target_node_id)
        await asyncio.sleep(2)
        staged_stats = await get_container_stats(temp_name, target_node_id)
        if str(staged_stats.get('status', '')).lower() != 'running':
            raise RuntimeError(f"Staged VPS failed health check: {staged_stats.get('status')}")
        await execute_lxc(temp_name, f"stop {temp_name}", node_id=target_node_id)

        # Cut-over only after the target copy passed validation.
        await execute_lxc(container_name, f"delete {container_name} --force", node_id=node_id)
        ...
        # Rollback copy is no longer needed once the new VPS is running.
        if rollback_created:
            try:
                await execute_lxc(rollback_name, f"delete {rollback_name} --force", node_id=node_id)
                rollback_created = False
            except Exception:
                logger.warning("Migration succeeded but rollback copy %s could not be removed", rollback_name)

        await send_ui(ctx, embed=create_success_embed(
            "VPS Migrated",
            f"Successfully migrated VPS `{container_name}` to node {target_node['name']}."
        ))
    except Exception as e:
        logger.exception("Migration failed for %s", container_name)

        if staged:
            try:
                await execute_lxc(temp_name, f"delete {temp_name} --force", node_id=target_node_id)
            except Exception:
                logger.warning("Failed to clean staged migration container %s", temp_name)

        # If the source name was removed during cut-over, restore it from the
        # rollback copy. If it was never removed, simply start the source again.
        try:
            await execute_lxc(container_name, f"info {container_name}", node_id=node_id)
            await execute_lxc(container_name, f"start {container_name}", node_id=node_id)
        except Exception:
            if rollback_created:
                try:
                    await execute_lxc(rollback_name, f"rename {rollback_name} {container_name}", node_id=node_id)
                    rollback_created = False
                    await execute_lxc(container_name, f"start {container_name}", node_id=node_id)
                    logger.warning("Migration rollback restored %s on source node", container_name)
                except Exception:
                    logger.exception("CRITICAL: failed to restore migration rollback for %s", container_name)

        await send_ui(ctx, embed=create_error_embed(
            "Migration Failed",
            f"Migration was aborted safely. The original VPS was preserved or restored whenever possible.\n\n**Error:** {str(e)}"
        ))
...
async def vps_stats(ctx, container_name: str):
    node_id = find_node_id_for_container(container_name)
    await send_ui(ctx, embed=create_info_embed("Gathering Statistics", f"Collecting statistics for VPS `{container_name}`..."))
    try:
        stats = await get_container_stats(container_name, node_id)
        ...
        await send_ui(ctx, embed=create_error_embed("Statistics Failed", f"Error: {str(e)}"))
...
async def vps_network(ctx, container_name: str, action: str, value: str = None):
    ...
    if action.lower() not in ["list", "add", "remove", "limit"]:
        await send_ui(ctx, embed=create_error_embed("Invalid Action", f"Use: `{PREFIX}vps-network <container> <list|add|remove|limit> [value]`"))
        return
    try:
        if action.lower() == "list":
            ...
            await send_ui(ctx, embed=create_success_embed("Network Limited", f"Set network limit to {value} for `{container_name}`"))
            ...
            await send_ui(ctx, embed=create_success_embed("Network Added", f"Added network interface to VPS `{container_name}` with bridge `{value}`"))
            ...
            await send_ui(ctx, embed=create_success_embed("Network Removed", f"Removed network interface `{value}` from VPS `{container_name}`"))
        else:
            await send_ui(ctx, embed=create_error_embed("Invalid Parameters", "Please provide valid parameters for the action"))
    except Exception as e:
        await send_ui(ctx, embed=create_error_embed("Network Management Failed", f"Error: {str(e)}"))

@bot.command(name='vps-processes')
@is_admin()
async def vps_processes(ctx, container_name: str):
    node_id = find_node_id_for_container(container_name)
    await send_ui(ctx, embed=create_info_embed("Gathering Processes", f"Listing processes in VPS `{container_name}`..."))
    try:
        ...
        await send_ui(ctx, embed=create_error_embed("Process Listing Failed", f"Error: {str(e)}"))

@bot.command(name='vps-logs')
@is_admin()
async def vps_logs(ctx, container_name: str, lines: int = 50):
    node_id = find_node_id_for_container(container_name)
    await send_ui(ctx, embed=create_info_embed("Gathering Logs", f"Fetching last {lines} lines from VPS `{container_name}`..."))
    try:
        ...
        await send_ui(ctx, embed=create_error_embed("Log Retrieval Failed", f"Error: {str(e)}"))
...
async def vps_password(ctx, container_name: str = None):
    """Regenerate a VPS root password without ever storing or displaying it publicly."""
    if not container_name:
        await send_ui(ctx, embed=create_info_embed(
            "Password Storage Disabled",
            "For security, VPS root passwords are never stored by the bot. "
            f"Use `{PREFIX}vps-password <container>` to generate a new password and send it privately to the owner."
        ))
        return
    ...
    if found_vps.get('suspended'):
        await send_ui(ctx, embed=create_error_embed("VPS Suspended", "Unsuspend the VPS before regenerating its root password."))
        return

    new_password = generate_strong_password()
    success, result = await configure_ssh(container_name, found_vps.get('node_id', 1), new_password)
    if not success:
        await send_ui(ctx, embed=create_error_embed("Password Regeneration Failed", str(result)))
        return

    try:
        owner = await bot.fetch_user(int(found_user_id))
        dm_embed = create_success_embed(
            "🔐 VPS Password Regenerated",
            f"A new root password has been generated for `{container_name}`. This password is not stored by the bot."
        )
        add_field(dm_embed, "SSH Username", "`root`", True)
        add_field(dm_embed, "New Password", f"`{new_password}`", False)
        add_field(dm_embed, "Security", "Save this password securely. Once this message is unavailable, the bot cannot recover it; regenerate it again if needed.", False)
        await send_ui(owner, embed=dm_embed)
        await send_ui(ctx, embed=create_success_embed(
            "Password Regenerated",
            f"A new root password was generated for `{container_name}` and sent privately to the VPS owner. It was not stored."
        ))
    except discord.Forbidden:
        await send_ui(ctx, embed=create_error_embed(
            "DM Failed",
            "The new password was generated but Discord could not DM the VPS owner. The password was not stored. Regenerate it again after the owner's DMs are enabled."
        ))
...
async def suspend_vps(ctx, container_name: str, *, reason: str = "Admin action"):
    ...
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if vps.get('status') != 'running':
                    await send_ui(ctx, embed=create_error_embed("Cannot Suspend", "VPS must be running to suspend."))
                    return
                try:
                    await execute_lxc(container_name, f"stop {container_name}", node_id=node_id)
                    vps['status'] = 'stopped'
                    ...
                    await send_ui(ctx, embed=create_error_embed("Suspend Failed", str(e)))
                    return
                try:
                    ...
                    await send_ui(owner, embed=embed)
                except Exception as dm_e:
                    logger.error(f"Failed to DM owner {uid}: {dm_e}")
                await send_ui(ctx, embed=create_success_embed("VPS Suspended", f"VPS `{container_name}` suspended. Reason: {reason}"))
                found = True
                break
        if found:
            break
    if not found:
        await send_ui(ctx, embed=create_error_embed("Not Found", f"VPS `{container_name}` not found."))
...
async def unsuspend_vps(ctx, container_name: str):
    ...
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                if not vps.get('suspended', False):
                    await send_ui(ctx, embed=create_error_embed("Not Suspended", "VPS is not suspended."))
                    return
                try:
                    vps['suspended'] = False
                    vps['status'] = 'running'
                    ...
                    await recreate_port_forwards(container_name)
                    save_vps_data()
                    await send_ui(ctx, embed=create_success_embed("VPS Unsuspended", f"VPS `{container_name}` unsuspended and started."))
                    found = True
                except Exception as e:
                    await send_ui(ctx, embed=create_error_embed("Start Failed", str(e)))
                try:
                    owner = await bot.fetch_user(int(uid))
...
async def suspension_logs(ctx, container_name: str = None):
    if container_name:
        ...
        if not history:
            await send_ui(ctx, embed=create_info_embed("No Suspensions", f"No suspension history for `{container_name}`."))
            return
        ...
        if not all_logs:
            await send_ui(ctx, embed=create_info_embed("No Suspensions", "No suspension events recorded."))
            return
        logs_text = "\n".join(all_logs)
...
async def apply_permissions(ctx, container_name: str):
    node_id = find_node_id_for_container(container_name)
    await send_ui(ctx, embed=create_info_embed("Applying Permissions", f"Applying advanced permissions to `{container_name}`..."))
    try:
        ...
        for user_id, vps_list in vps_data.items():
            for vps in vps_list:
                if vps['container_name'] == container_name:
                    ...
                    save_vps_data()
                    break
        await send_ui(ctx, embed=create_success_embed("Permissions Applied", f"Advanced permissions applied to VPS `{container_name}`. Docker-ready with unprivileged ports!"))
    except Exception as e:
        await send_ui(ctx, embed=create_error_embed("Apply Failed", f"Error: {str(e)}"))
...
async def resource_check(ctx):
    ...
    msg = await send_ui(ctx, embed=embed)
    for user_id, vps_list in vps_data.items():
        for vps in vps_list:
                ...
                if cpu > CPU_THRESHOLD or ram > RAM_THRESHOLD:
                    ...
                    try:
                        ...
                        try:
                            ...
                            await send_ui(owner, embed=warn_embed)
                        except Exception as dm_e:
    ...
    await edit_ui(msg, embed=final_embed)

@bot.command(name='whitelist-vps')
@is_admin()
async def whitelist_vps(ctx, container_name: str, action: str):
    if action.lower() not in ['add', 'remove']:
        await send_ui(ctx, embed=create_error_embed("Invalid Action", f"Use: `{PREFIX}whitelist-vps <container> <add|remove>`"))
        return
    found = False
    for user_id, vps_list in vps_data.items():
        for vps in vps_list:
            if vps['container_name'] == container_name:
                ...
                save_vps_data()
                await send_ui(ctx, embed=create_success_embed("Whitelist Updated", f"VPS `{container_name}` {msg}."))
                found = True
                break
        if found:
            break
...
async def backup_db(ctx):
    ...
    try:
        ...
        await send_ui(ctx, embed=create_success_embed("DB Backup Created", f"Backup saved as `{backup_name}` (and WAL/SHM if applicable)."))
    except Exception as e:
        await send_ui(ctx, embed=create_error_embed("Backup Failed", f"Error: {str(e)}"))

@bot.command(name='repair-ports')
@is_admin()
async def repair_ports(ctx, container_name: str):
    await send_ui(ctx, embed=create_info_embed("Repairing Ports", f"Re-adding port forward devices for `{container_name}`..."))
    try:
        readded = await recreate_port_forwards(container_name)
        await send_ui(ctx, embed=create_success_embed("Ports Repaired", f"Re-added {readded} port forwards for `{container_name}`."))
    except Exception as e:
        await send_ui(ctx, embed=create_error_embed("Repair Failed", f"Error: {str(e)}"))
...
async def set_expiration(ctx, container_name: str, days: int):
    ...
    if days <= 0:
        await send_ui(ctx, embed=create_error_embed("Invalid Days", "Days must be a positive number."))
        return
...
async def node_cmd(ctx, sub: str, *args):
    if sub == 'create':
        await send_ui(ctx, "Enter node name:")
        def check(m):
            return m.author == ctx.author and m.channel == ctx.channel
        name = (await bot.wait_for('message', check=check)).content.strip()
        await send_ui(ctx, "Enter location:")
        location = (await bot.wait_for('message', check=check)).content.strip()
        await send_ui(ctx, "Enter total VPS capacity:")
        total_vps_str = (await bot.wait_for('message', check=check)).content.strip()
        try:
            total_vps = int(total_vps_str)
        except ValueError:
            await send_ui(ctx, embed=create_error_embed("Invalid Input", "Total VPS must be an integer."))
            return
        await send_ui(ctx, "Enter tags (comma separated):")
        tags_str = (await bot.wait_for('message', check=check)).content.strip()
        tags = [t.strip() for t in tags_str.split(',') if t.strip()]
        tags_json = json.dumps(tags)
        await send_ui(ctx, "Enter node URL (e.g., http://ip:port or https://ip:port) or leave blank for local:")
        ...
        try:
            ...
            await send_ui(ctx, embed=create_error_embed("Error", "Node name already exists."))
        conn.close()
    elif sub == 'list':
        nodes = get_nodes()
        ...
        if not args:
            await send_ui(ctx, embed=create_error_embed("Usage", f"{PREFIX}node edit <id>"))
            return
        try:
            node_id = int(args[0])
        except ValueError:
            await send_ui(ctx, embed=create_error_embed("Invalid ID", "Node ID must be an integer."))
            return
        node = get_node(node_id)
        if not node:
            await send_ui(ctx, embed=create_error_embed("Not Found", "Node not found."))
            return
        await send_ui(ctx, f"Editing node {node['name']}. Enter new name ( . to skip):")
        def check(m):
            return m.author == ctx.author and m.channel == ctx.channel
        new_name = (await bot.wait_for('message', check=check)).content.strip()
        if new_name != '.':
            node['name'] = new_name
        await send_ui(ctx, "New location ( . to skip):")
        new_loc = (await bot.wait_for('message', check=check)).content.strip()
        if new_loc != '.':
            node['location'] = new_loc
        await send_ui(ctx, "New total VPS capacity ( . to skip):")
        new_total = (await bot.wait_for('message', check=check)).content.strip()
        if new_total != '.':
            node['total_vps'] = int(new_total)
        await send_ui(ctx, "New tags (comma separated, . to skip):")
        ...
        if node['is_local']:
            await send_ui(ctx, "Convert Local Node to Dynamic URL-based Node? (y/n):")
            convert = (await bot.wait_for('message', check=check)).content.strip().lower()
            if convert == 'y':
                await send_ui(ctx, "Enter node URL (e.g., http://ip:port or https://ip:port):")
                url_str = (await bot.wait_for('message', check=check)).content.strip()
                if not url_str:
                    await send_ui(ctx, embed=create_error_embed("Error", "URL cannot be empty for dynamic node."))
                    return
                ...
                await send_ui(ctx, f"✅ Node converted to Dynamic!\n\n**URL:** `{url_str}`\n**Generated API Key:** `{node['api_key']}`\n\n**Setup Command:**\n```\npython node-agent.py --api_key={node['api_key']} --port=PORT\n```")
        else:
            await send_ui(ctx, "Convert Dynamic Node to Local? (y/n):")
            convert = (await bot.wait_for('message', check=check)).content.strip().lower()
            if convert == 'y':
                node['url'] = None
                node['api_key'] = None
                node['is_local'] = 1
                await send_ui(ctx, "✅ Node converted to Local!")
            else:
                await send_ui(ctx, "New URL ( . to skip):")
                ...
                if new_url != '.':
                    ...
                    node['url'] = new_url
                await send_ui(ctx, "Regenerate API key? (y/n):")
    ...
    # Delete subcommand
        ...
        if not args:
            await send_ui(ctx, embed=create_error_embed("Usage", f"{PREFIX}node delete <id> [force]"))
            return
        ...
        if len(args) > 1 and args[1].lower() == 'force':
            ...
            await send_ui(ctx, embed=create_error_embed("Invalid Argument", "Optional argument must be 'force'."))
            return
        ...
        if node['is_local']:
            await send_ui(ctx, embed=create_error_embed("Cannot Delete", "Cannot delete the local node."))
            return

        # Check if node has any VPS assigned
        vps_count = get_current_vps_count(node_id)
        if not force and vps_count > 0:
            await send_ui(ctx, embed=create_error_embed("Cannot Delete",
        ...
        class ConfirmDelete(AppLayoutView):
            ...
            async def confirm(self, inter: discord.Interaction, item: discord.ui.Button):
                ...
                await send_ui(inter.followup, embed=success_embed)
                self.stop()
            ...
            async def cancel(self, inter: discord.Interaction, item: discord.ui.Button):
                ...
                await edit_ui(inter.response,
                    embed=create_info_embed("Deletion Cancelled", "Node deletion was cancelled."),
                    view=None
                )
                self.stop()

        await send_ui(ctx, embed=embed, view=ConfirmDelete(node_id, node['name'], force, vps_count))

    elif sub == 'status':
        # Check node status
        if not args:
            await send_ui(ctx, embed=create_error_embed("Usage", f"{PREFIX}node status <id>"))
            return
        ...
        # Regenerate API key for dynamic node
        if not args:
            await send_ui(ctx, embed=create_error_embed("Usage", f"{PREFIX}node regen-key <id>"))
            return
        ...
        if node['is_local']:
            await send_ui(ctx, embed=create_error_embed("Error", "Cannot regenerate API key for Local nodes. Only Dynamic nodes have API keys."))
            return
        ...
        await send_ui(ctx, embed=warning_embed, view=ConfirmRegenKey(node_id, node))

    elif sub == 'migrate':
        # NEW: this subcommand is now real. Previously the help text
        # advertised "{PREFIX}node migrate <from> <to>" as a subcommand of
        # !node, but no such subcommand existed — only the separate
        # !migrate-vps <container> <target_node_id> command did the work.
        # This adds a thin, documented wrapper so the advertised syntax
        # actually functions: !node migrate <container> <target_node_id>
        if len(args) < 2:
            await send_ui(ctx, embed=create_error_embed("Usage", f"{PREFIX}node migrate <container_name> <target_node_id>"))
            return
        container_name = args[0]
        try:
            target_node_id = int(args[1])
        except ValueError:
            await send_ui(ctx, embed=create_error_embed("Invalid ID", "Target node ID must be an integer."))
            return
        await migrate_vps(ctx, container_name, target_node_id)

    else:
        # Show help for node command
        embed = create_info_embed("Node Management",
            f"Manage multi-node infrastructure for {BOT_NAME}")
        add_field(embed, "Subcommands",
            f"`{PREFIX}node create` - Create a new node\n"
            f"`{PREFIX}node list` - List all nodes\n"
            f"`{PREFIX}node status <id>` - Check node status\n"
            f"`{PREFIX}node edit <id>` - Edit node details\n"
            f"`{PREFIX}node delete <id> [force]` - Delete a node\n"
            f"`{PREFIX}node regen-key <id>` - Regenerate a Dynamic node's API key\n"
            f"`{PREFIX}node migrate <container> <target_node_id>` - Migrate a VPS to another node",
            False)
        await send_ui(ctx, embed=embed)
...
class HelpView(AppLayoutView):
        ...
        self.command_categories = {
            ...
            "vps": {
                ...
                "commands": [
                    ...
                    (f"{PREFIX}vps-status <vps-id>", "Get VPS status (running/stopped) — quick check"),
                    ...
                    (f"{PREFIX}vps-password <vps-id>", "Regenerate VPS root password (never stored)"),
            ...
            "nodes": {
                ...
                "commands": [
                    ...
                    (f"{PREFIX}node migrate <container> <target_node_id>", "Migrate VPS between nodes (Admin only)"),
            ...
            "admin": {
                ...
                "commands": [
                    ...
                    (f"{PREFIX}migrate-vps <vps-id> <target_node_id>", "Migrate VPS to another node"),
                    ...
                    (f"{PREFIX}vps-password <vps-id>", "Regenerate VPS password (never stored)"),
                    (f"{PREFIX}node-check <node_id>", "Check node health and status"),
                    (f"{PREFIX}status", "Full system status dashboard (no arguments)"),
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if interaction.user != self.ctx.author:
            await send_ui(interaction.response, "This menu is not for you!", ephemeral=True)
            return

        self.current_category = interaction.data['values'][0]
        self.update_embed()
        await edit_ui(interaction.response, embed=self.embed, view=self)
...
async def show_help(ctx):
    ...
    view = HelpView(ctx)
    await send_ui(ctx, embed=view.embed, view=view)


# Command aliases for typos and convenience
@bot.command(name='mangage')
async def manage_typo(ctx):
    await send_ui(ctx, embed=create_info_embed("Command Correction", f"Did you mean `{PREFIX}manage`? Use the correct command."))
...
async def stats_alias(ctx):
    if str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) in admin_data.get("admins", []):
        ...
        await send_ui(ctx, embed=create_error_embed("Access Denied", "This command requires admin privileges."))
...
async def info_alias(ctx, user: discord.Member = None):
    if str(ctx.author.id) == str(MAIN_ADMIN_ID) or str(ctx.author.id) in admin_data.get("admins", []):
        if user:
            ...
            await send_ui(ctx, embed=create_error_embed("Usage", f"Please specify a user: `{PREFIX}info @user`"))
    else:
        await send_ui(ctx, embed=create_error_embed("Access Denied", "This command requires admin privileges."))
...
if __name__ == "__main__":
    if not DISCORD_TOKEN:
        logger.error("❌ ERROR: No DISCORD_TOKEN found in the environment / .env file.")
        logger.error("Add DISCORD_TOKEN=your_token_here to your .env file and restart.")
        exit(1)
    try:
